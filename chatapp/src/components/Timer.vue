...
<template>
  <div class="timer">
    ...
    <v-btn v-on:click="start" v-if="!timerOn" color="primary" elevation="2">スタート</v-btn>
    <v-btn v-on:click="stop" v-if="timerOn" color="primary" elevation="2">ストップ</v-btn>
    <!-- 追加ボタン（add関数を呼び出し）を表示 -->
    <v-btn v-on:click="add" class="addButton" color="primary" elevation="2">１分追加</v-btn>
    <v-btn v-on:click="complete" class="completeButton" color="primary" elevation="2">終了</v-btn>
  </div>
</template>
